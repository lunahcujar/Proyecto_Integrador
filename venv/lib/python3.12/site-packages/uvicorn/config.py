...
class Config:
    ...
    def load(self) -> None:
        ...
        try:
            ...
            logger.error("Error loading ASGI app_. %s" % exc)
            sys.exit(1)
        ...
        try:
            ...
            if self.factory:
                logger.error("Error loading ASGI app_ factory: %s", exc)
                sys.exit(1)
        else:
            if not self.factory:
                logger.warning(
                    "ASGI app_ factory detected. Using it, but please consider setting the --factory flag explicitly."
                )
